...
| **CI** | `lint`, `test`, `test-postgres`, `hygiene`, `audit` | PR and push to main | none |
| **branch rule** | the GitHub "Defaults" ruleset on `main`: no deletion, no force-push, PR required, the five checks required | merge | none |
...
the `lint` job, the rest by `hygiene` over `pre-commit run --all-files`), so a
`--no-verify` push costs time, not correctness.
...
`audit`). Adding a `name:` to a job renames the check and silently breaks the
required-checks rule; don't.
...
- **A coverage threshold.** SimpleCov prints one `Coverage: NN.N% (a/b lines)`
  line and CI lifts it into the job summary; nothing fails on it. Currently
  90.5%. A number that is a gate gets gamed with tests that assert nothing.
...
- **A stage** if it is a Ruby-level check over the project (a new linter, a
  second test runner). Add it to the `case` in `run_stage`, to the validation
  list, to `usage`, and to `DEFAULT_STAGES` unless it needs the network or a
  service — then give it its own CI job the way `audit` has one.
...
- `.github/workflows/ci.yml` — the five jobs, each one calling `bin/ci`.
